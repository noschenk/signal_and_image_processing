...
def blur_and_downsample(img, sigma, filter_size, scale):
    ...
    # filter the image with a gaussian filter of given size and sigma
    img_br_ds = gconv(img, sigma, filter_size)
    # downscale the blurred image
    img_br_ds = misc.imresize(img_br_ds, scale)
    return img_br_ds
...
def generate_gaussian_pyramid(img, sigma, filter_size, scale, num_levels):
    ...
    # @ scale               : Downscaling factor (of type float, between 0-1)
    # OUTPUTS
    # @ gaussian_pyramid    : A list containing images of pyramid. The first
    ...
    # add the original image at level 0
    gaussian_pyramid.append(img)
    # add the following images at following levels (number of levels incl. original image = num_levels)
    #  using the previously defined function blur_and_downsample
    for i in range(1, num_levels):
        res = blur_and_downsample(gaussian_pyramid[(i - 1)], sigma, filter_size, scale)
